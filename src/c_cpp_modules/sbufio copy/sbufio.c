...
struct sbuf *sbufio_get(struct sbufio *const restrict sbio, const off_t id)
{
    ...
    {
        idx_page->nref++;
        *sbuf_p = db_sbuf_at(sbio, *(void **)(idx_page->buf + p_offset));
    };
    return *sbuf_p;
};

struct sbuf *sbufio_set(struct sbufio *const restrict sbio, const off_t id, struct sbuf *const restrict sbuf)
{
    struct sbufio_idx_page *idx_page;
    off_t p_offset = idx_locate_to_id(sbio->idx, id, &idx_page);
    struct sbuf **sbuf_p = idx_page->sbs + p_offset;
...
struct sbuf *sbufio_alloc(struct sbufio *const restrict sbio, const off_t size){

};

void sbufio_free(struct sbufio *const restrict sbio, struct sbuf *const restrict sb){
